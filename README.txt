...
Parallelized, multi-format (WebP + AVIF) image conversion for WordPress. A fork of WebP Express.

== Description ==

Magic Convert is a fork of [WebP Express](https://github.com/rosell-dk/webp-express) by Bjørn Rosell, extended for parallelized bulk conversion of large media libraries, AVIF output alongside WebP, and native nginx config generation. The original WebP Express functionality and documentation are preserved below; the rebrand and new capabilities build on that foundation. Many thanks to Bjørn Rosell and the WebP Express contributors for the excellent groundwork.
...
*wp magic-convert convert --only-png*: Only the PNG images
*wp magic-convert convert --quality=50*: Use quality 50 (instead of what was entered in settings screen)
*wp magic-convert convert --quality=50 --near-lossless=50 --alpha-quality=50 --encoding=lossy*: More conversion options. encoding can be "lossy", "lossless" or "auto"
*wp magic-convert convert --converter=cwebp*: Specifically use cwebp converter. Other options: "vips", "imagemagick", "ffmpeg". PS: For "ewww" and "wpc" (remote webp express) does not work here.
...
= 0.20.0 =
* Added WP CLI support. Add "wp magic-convert convert" to crontab for nightly conversions of new images.
