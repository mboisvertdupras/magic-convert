...
(function () {
    ...
    var PAGE_FETCH_RETRY_MS = 3000;     // throttle before retrying a failed list-page fetch
    var PAGE_FETCH_MAX_RETRIES = 5;     // give up paging (and finish) after this many in a row
    var GROWTH_COOLDOWN_MS = 20000;     // after a backoff, don't grow the pool again for this long
    ...
    // The adaptive growth ceiling for the pool.
    //
    // A manual Advanced override is an explicit hard cap the user owns (bounded only by
    // the absolute max). In automatic mode the ceiling is resource-aware and SELF-TUNING:
    //   - never above the absolute max or the server's stated max,
    //   - never above the server's resource recommendation (≈ cores/2) — except we ignore
    //     a recommendation of 1, which is also what the advisor returns when the server
    //     merely happened to be busy at dialog-open; we don't want a transient blip to pin
    //     the whole run to a single worker,
    //   - never above the soft ceiling the run LEARNS the first time the server signals
    //     "busy" (see applyBackoff). This is what makes the pool CONVERGE to a sustainable
    //     concurrency instead of ramping to the max, overloading, backing off, and
    //     repeating — the ~70-files-then-stall oscillation.
    function growthCeiling() {
        ...
        var cap = Math.min(ABSOLUTE_MAX_WORKERS, (run && run.serverMax) || ABSOLUTE_MAX_WORKERS);
        if (run && run.recommended >= 2) { cap = Math.min(cap, run.recommended); }
        if (run && run.softCeiling) { cap = Math.min(cap, run.softCeiling); }
    ...
    function startRun(firstResponse, recommended, serverMax) {
        // Abandon any prior run (e.g. the modal was closed mid-run and reopened) so its
        // still-pending worker callbacks and refill/page-retry timers cannot bleed into
        // this fresh run and double-drive the pool. The run-identity checks in the timer
        // callbacks honour this flag; resetting the coalescing latch lets this run arm
        // its own refills immediately instead of waiting on the prior run's stale timer.
        ...
        run = {
            ...
            pageFetchFailures: 0,                           // consecutive list-page fetch failures
            ...
            recommended: recommended,                        // resource-aware auto ceiling (≈ cores/2)
            softCeiling: 0,                                  // learned ceiling (0 = not learned yet)
            lastBusyAt: 0,                                   // ts of last backoff (for growth cooldown)
            activeWorkers: 0,
            successStreak: 0,
            paused: false,
            stopped: false,
    ...
    function updateProgressUi() {
        // Once finishRun() has painted the terminal "Done" summary, do not repaint:
        // fillPool() calls updateProgressUi() right after finishRun(), and without this
        // guard it would clobber "Done" back to "Converting with 0 parallel workers",
        // leaving every completed run looking like it is still (or stuck) converting.
    ...
    function maybeFetchMorePages() {
        ...
        fetchPage(run.listId, page).then(function (data) {
            ...
            if (data && data.success && data.files && data.files.length) {
                ...
                // Server reports no more files: the list is drained. Re-drive the pool
                // so fillPool()'s completion check can fire — it never runs on its own
                // once the pool has gone idle behind this in-flight fetch, which would
                // otherwise leave the run stuck at ~100% and never showing "Done".
                run.pagesExhausted = true;
                fillPool();
            }
        }).catch(function () {
            ...
            if (run.pageFetchFailures >= PAGE_FETCH_MAX_RETRIES) {
                // Repeated paging failures: finish with what converted so far and
                // surface the problem rather than retrying (and stalling) forever.
                run.pagesExhausted = true;
                run.errors.push({
                    path: '(more files)',
                    format: '',
            ...
            // Transient failure: retry after a throttled delay. The old code cleared
            // pageFetchInFlight but never re-drove the pool, so a single failed page
            // fetch stranded the whole run with work still pending. Capture the run so a
            // retry that outlives its run (modal closed / new run started) is dropped.
    ...
    function fillPool() {
        ...
        // Liveness guard. Once the pool goes quiescent (activeWorkers === 0) there is
        // no worker left to re-enter the loop, so we must either finish or guarantee a
        // future wake-up — otherwise the run stalls forever with work still pending.
        if (run.activeWorkers === 0) {
            if (run.queue.length === 0 && run.pagesExhausted && !run.pageFetchInFlight) {
                ...
                // Work waiting, but we are inside a backoff window with no worker to
                // re-trigger the pool. The refill timer that delivered us here can fire
                // EARLY: several workers hitting server_busy in one tick each extend
                // run.backoffUntil, but scheduleRefill() coalesces, so the single armed
                // timer aims at the first (shortest) deadline. It fires before
                // backoffUntil, the while-loop above spawns nothing, and without this
                // re-arm nothing ever retries — the "Server busy — backing off…" hang.
                scheduleRefill();
            }
            // The remaining quiescent state (queue empty, pages still pending behind an
            // in-flight fetch) is re-driven by maybeFetchMorePages()'s settled handlers.
        }
    ...
    function registerSuccess() {
        ...
        if (run.successStreak >= SUCCESS_STREAK_TO_GROW) {
            run.successStreak = 0;
            // Hysteresis: never re-grow within one cooldown window of a backoff. The busy
            // signal is the server's 1-minute load average, which lags real load by tens of
            // seconds; growing again immediately just chases that stale sensor straight back
            // into overload — the engine of the ~70-files-then-stall oscillation.
            if ((Date.now() - run.lastBusyAt) < GROWTH_COOLDOWN_MS) { return; }
            if (run.desired < growthCeiling()) {
                run.desired++;
            }
        }
    }
    ...
    function applyBackoff() {
        var now = Date.now();
        // Learn a soft ceiling on the FIRST backoff of an episode — capturing the peak
        // 'desired' that overloaded the server — so the pool converges to just below that
        // point and stops slamming into it. The cooldown guard means a same-tick burst of
        // backoffs (or a re-trip moments later) does not ratchet the ceiling down too far.
        if ((now - run.lastBusyAt) > GROWTH_COOLDOWN_MS) {
            run.softCeiling = Math.max(1, run.desired - 1);
        }
        run.lastBusyAt = now;
    ...
    function scheduleRefill() {
        ...
        setTimeout(function () {
            refillScheduled = false;
            // Ignore a timer that outlived its run (modal closed, or a new run started).
            if (run === scheduledRun && !run.stopped) { fillPool(); }
        }, Math.max(0, delay));
    }
