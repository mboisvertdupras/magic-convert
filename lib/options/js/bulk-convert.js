...
function openBulkConvertPopup() {
    ...
    var data = {
        ...
        'nonce' : window.magicConvert['ajax-nonces']['list-unconverted-files'],
    };
    jQuery.ajax({
      type: "POST",
      url: ajaxurl,
      ...
      success: function(response) {
          ...
          window.magicconvert_bulkconvert = bulkInfo;
...
function pauseBulkConversion() {
    var bulkInfo = window.magicconvert_bulkconvert;
...
function magicconvert_viewLog(groupPointer, filePointer) {
...
/*
    ...
    jQuery.ajax({
        ...
        url: ajaxurl,
        data: {
            'action': 'magicconvert_view_log',
            'nonce' : window.magicConvert['ajax-nonces']['view-log'],
            'source': source
        },
        success: (response) => {
            //alert(response);
            ...
            result = magicconvert_escapeHTML(result);
...
function convertNextInBulkQueue() {
    ...
    function responseCallback(response){
        ...
        //var htmlViewLog = '&nbsp;&nbsp;<a style="cursor:pointer" onclick="magicconvert_viewLog(' + bulkInfo.groupPointer + ',' + bulkInfo.filePointer  + ')">view log</a>';
        if (result['success']) {
            ...
            if (result['new-convert-nonce']) {
                ...
                window.magicConvert['ajax-nonces']['convert'] = result['new-convert-nonce'];
            }
...
/*
            ...
            if (result['msg']) {
                logLn(html);
                logLn('<br><br><span style="color:red; font-size:15px">' + magicconvert_escapeHTML(result['msg']) + '</span>');
            }
            if (result['stop']) {
                return;
            }
