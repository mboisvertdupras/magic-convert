...
// AVIF converter list — the AVIF counterpart to converters.js.
//
// Drives the SEPARATE AVIF converter stack: window.avifConverters <-> the hidden
// input[name='avif-converters'] (sanitized server-side into config.formats.avif.converters and
// consumed by AvifStack::fromConverterList()). It deliberately does NOT offer a per-converter
// "configure" dialog: AVIF converters take only the global quality/speed knobs in the AVIF section,
// not per-converter options — so the list offers reorder + activate/deactivate + test + live status.
//
// Entries arrive pre-annotated with {working, error} (computed server-side from AvifStack::selfTest)
// so each row can render its ✓/✗ status without an extra round-trip.
...
function generateAvifConverterHTML(converter) {
    ...
    // drag handle (identical to the WebP converter list)
...
/* AVIF converter ids are unique, so the temporary id is simply the converter id. */
...
/* Update the hidden input containing the AVIF converter list.
 * Only the persisted shape {converter[, deactivated]} is written; the transient render-only fields
 * (id / working / error) are stripped (the server sanitizer would drop them anyway, but a clean
 * payload keeps the saved config tidy). */
...
function setAvifConvertersHTML() {
    ...
    if (el == null) {
        // The AVIF section is not on this page (e.g. an operation mode without it). Nothing to do.
        return;
    }
...
/* Test link: encode a sample image with JUST the clicked AVIF converter and show the result.
 * Reuses the shared convert_file admin-ajax action with format=avif; the server forces the AVIF
 * stack to this single converter (see Convert::processAjaxConvertFile). */
...
function renderAvifTestResult(response, label) {
    ...
    if (result['success'] === true) {
        ...
        var avifSize = result['filesize-webp']; // converted size (key name is legacy/webp)
        if (orgSize && avifSize) {
