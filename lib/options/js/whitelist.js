...
function whitelistStartPolling() {
    ...
    jQuery.post(window.ajaxurl, {
            'action': 'magicconvert_start_listening',
        }, function(response) {
            window.whitelistTid = window.setInterval(function() {
                jQuery.post(window.ajaxurl, {
                        'action': 'magicconvert_get_request',
                    }, function(response) {
                        if (response && (response.substr(0,1) == '{')) {
                            var r = JSON.parse(response);
                            window.magicconvert_incoming_request = r;
                            //console.log(r);
...
function whitelistCancelListening() {
    /*
    jQuery.post(window.ajaxurl, {
            'action': 'magicconvert_stop_listening',
        }, function(response) {}
    );
    */
}
...
function whitelistAcceptRequest() {
    ...
    var r = window.magicconvert_incoming_request;
...
function whitelistSetHTML() {
    ...
    if (window.whitelist && window.whitelist.length > 0) {
        ...
        for (var i=0; i<window.whitelist.length; i++) {
            s+='<li>';
            s+= magicconvert_escapeHTML(window.whitelist[i]['label']);
