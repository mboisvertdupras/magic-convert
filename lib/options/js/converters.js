      ...
      html += '</g></svg>';
        ...
        if (converter['warnings'].join) {
            ...
            for (var i = 0; i<converter['warnings'].length; i++) {
                html += '<p>' + magicconvert_escapeHTML(converter['warnings'][i]) + '</p>';
            }
...
function wpcRequestAccess() {
    ...
    url = 'http://we0/wordpress/magic-convert-server';

    jQuery.post(window.ajaxurl, {
        'action': 'magicconvert_request_access',
    }, function(response) {
