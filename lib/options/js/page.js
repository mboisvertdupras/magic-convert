...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!window.magicConvert['can-use-doc-root-for-structuring']) {
    ...
    // Toggle AVIF advanced options (Quality / Encoding speed) — revealed only when AVIF is enabled.
    // Mirrors the alter-html / web-service reveal pattern: the row state is driven entirely by the
    // checkbox's DOM value, so no separate JS config mirror is needed.
    if (el('avif_enabled') && el('avif_options_div')) {
        function updateAvifOptionsVisibility() {
            toggleVisibility('avif_options_div', el('avif_enabled').checked);
        }
        updateAvifOptionsVisibility();
        el('avif_enabled').addEventListener('change', function() {
            updateAvifOptionsVisibility();
        });
    }
    ...
    document.getElementById('change_operation_mode').addEventListener('change', function() {
        ...
        if (confirm(msg)) {
            document.getElementById('magicconvert_settings').submit();
        } else {
