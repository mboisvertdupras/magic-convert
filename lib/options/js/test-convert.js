function openTestConvertPopup(converterId) {
    ...
    magicconvert_createTCOptions(converterId);
}

function magicconvert_createTCOptions(converterId) {
...
//    html += '<div id="tc_jpeg_options" class="toggler effect-visibility"></div>';
    ...
    document.querySelectorAll('#tc_conversion_options select').forEach(function(el) {
        el.addEventListener('change', function() {
            magicconvert_updateVisibilities();
        });
    });
...
function magicconvert_updateVisibilities() {
...
function runTestConversion() {
    ...
    var data = {
        ...
        'filename': window.magicConvertPaths['filePaths']['magicConvertRoot'] + '/test/' + elTxt('image'),
...
function convertResponseCallback(response){
    ...
    if (result['success'] === true) {
        ...
        if (window.canDisplayWebp) {
            ...
            var srcUrl = '/' + window.magicConvertPaths['urls']['magicConvertRoot']  + '/test/' + filename;
            ...
            /*
            var webpUrl = '/' + window.magicConvertPaths['urls']['content'] +
                              '/magic-convert/webp-images/doc-root/' +
                              window.magicConvertPaths['filePaths']['pluginRelToDocRoot'] + '/' +
                              'magic-convert/' +
                              'test/' +
                              filename + '.webp';
                          */
        ...
        if (result['msg'] != '') {
            html += ' <h3>Message: <span style="color:red; font-weight: bold">' + magicconvert_escapeHTML(result['msg']) + '</span></h3>';
        }
        if (result['log'] != '') {
            html += '<h3>Conversion log:</h3>';
            html += '<pre style="white-space:pre-wrap">' + magicconvert_escapeHTML(result['log']) + '</pre>';
