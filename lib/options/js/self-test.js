if (typeof MagicConvert == 'undefined') {
    MagicConvert = {};
}

MagicConvert.SelfTest = {
    'clear': function() {
        document.getElementById('magicconvert_test_redirection_content').innerHTML = '';
    },
    'write': function(html) {
        //var el = document.getElementById('magicconvert_test_redirection_content');
        //el.innerHTML += html;
        var el = document.createElement('div');
        el.innerHTML = html;
        document.getElementById('magicconvert_test_redirection_content').appendChild(el);
    },
    'simpleMdToHtml': function(line) {
        ...
        return '<div class="magicconvert md">' + line + '</div>';
    ...
    'responseHandler': function(response) {
        if ((typeof response == 'object') && (response['success'] == false)) {
            ...
            if (response['data'] && ((typeof response['data']) == 'string')) {
                html += magicconvert_escapeHTML(response['data']);
            }
            MagicConvert.SelfTest.write(html);
        ...
        for (var i=0; i<result.length; i++) {
            ...
            line = magicconvert_escapeHTML(line);
            line = MagicConvert.SelfTest.simpleMdToHtml(line);

            MagicConvert.SelfTest.write(line);
        }
        ...
        if (next == 'done') {
            //MagicConvert.SelfTest.write('<br>done');
        } else if (next == 'break') {
            MagicConvert.SelfTest.write('breaking');
        } else {
            MagicConvert.SelfTest.runTest(next);
        }

    },
    'runTest': function(testId) {
        var data = {
    		'action': 'magicconvert_self_test',
            'testId': testId,
            'nonce' : window.magicConvert['ajax-nonces']['self-test'],
    	};
        ...
        jQuery.ajax({
            ...
            url: ajaxurl,
            data: data,
            success: MagicConvert.SelfTest.responseHandler,
            error: function() {
                MagicConvert.SelfTest.write('PHP error. Check your debug.log for more info (make sure debugging is enabled)');
            },
        });
    },
    'openPopup': function(testId) {
        MagicConvert.SelfTest.clear();
        ...
        if (testId == 'nginxLiveTest') {
            title = 'Nginx rules live test';
        }
        tb_show(title, '#TB_inline?inlineId=magicconvert_test_redirection_popup&width=' + w + '&height=' + h);

        MagicConvert.SelfTest.runTest(testId);
