...
class WCFMPage
{
    ...
    public static function display() {
        echo '<div id="wcfmintro">' .
          '<h1>Magic Convert Conversion Browser</h1>' .
          '</div>';
...
//        print_r(WebPConvert::getConverterOptionDefinitions('png', false, true));
    ...
    public static function enqueueScripts() {
        $ver = '0';
        wp_register_script('wcfileman', plugins_url('js/wcfm/index.js', MAGIC_CONVERT_PLUGIN), [], $ver);
        wp_enqueue_script('wcfileman');
    }*/

    public static function addToHead() {
        $baseUrl = plugins_url('lib/wcfm', MAGIC_CONVERT_PLUGIN);
        //$url = plugins_url('js/conversion-manager/index.be5d792e.js  ', MAGIC_CONVERT_PLUGIN);

        $wcfmNonce = wp_create_nonce('magicconvert-wcfm-nonce');
        echo '<scr' . 'ipt>window.magicConvertWCFMNonce = "' . $wcfmNonce . '";</scr' . 'ipt>';
