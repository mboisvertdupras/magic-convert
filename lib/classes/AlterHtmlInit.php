...
class AlterHtmlInit
{
    ...
    public static function startOutputBuffer()
    {
        if (!is_admin() || (function_exists("wp_doing_ajax") && wp_doing_ajax()) || (defined( 'DOING_AJAX' ) && DOING_AJAX)) {
            ...
            ob_start('\\MagicConvert\\AlterHtmlInit::alterHtml');
        }
    }
        ...
        echo '<script>'
           ...
           . 'if(!window.HTMLPictureElement && document.addEventListener) {'
                . 'window.addEventListener("DOMContentLoaded", function() {'
                    ...
                    . 's.src = "' . plugins_url('/js/picturefill.min.js', MAGIC_CONVERT_PLUGIN) . '";'
    ...
    public static function setHooks() {
        ...
        if (Option::getOption('magic-convert-alter-html-add-picturefill-js')) {
            add_action( 'wp_head', '\\MagicConvert\\AlterHtmlInit::addPictureFillJs');
        }

        if (Option::getOption('magic-convert-alter-html-hooks', 'ob') == 'ob') {
            /* TODO:
            ...
            */
            add_action( 'init', '\\MagicConvert\\AlterHtmlInit::startOutputBuffer', 1 );
            add_action( 'template_redirect', '\\MagicConvert\\AlterHtmlInit::startOutputBuffer', 10000 );

        } else {
            add_filter( 'the_content', '\\MagicConvert\\AlterHtmlInit::alterHtml', 99999 ); // priority big, so it will be executed last
            add_filter( 'the_excerpt', '\\MagicConvert\\AlterHtmlInit::alterHtml', 99999 );
            add_filter( 'post_thumbnail_html', '\\MagicConvert\\AlterHtmlInit::alterHtml', 99999);
            add_filter( 'woocommerce_product_get_image', '\\MagicConvert\\AlterHtmlInit::alterHtml', 99999 );
            add_filter( 'get_avatar', '\\MagicConvert\\AlterHtmlInit::alterHtml', 99999 );
            add_filter( 'acf_the_content', '\\MagicConvert\\AlterHtmlInit::alterHtml', 99999 );
            add_action( 'dynamic_sidebar_before', '\\MagicConvert\\AlterHtmlInit::sidebarBeforeAlterHtml', 0 );
            add_action( 'dynamic_sidebar_after', '\\MagicConvert\\AlterHtmlInit::sidebarAfterAlterHtml', 1000 );
