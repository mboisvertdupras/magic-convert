    ...
    protected function runTestForImageType($rootId, $imageType)
    {
        ...
        if (!$success) {
            $log[] = 'The test cannot be completed, as the HTTP request failed. This does not neccesarily mean that the redirections ' .
                ...
                'You might also want to check out why a simple HTTP request could not be issued. Magic Convert uses such requests ' .
        ...
        if ($headers['content-type'] == 'image/webp') {
            ...
            $log[] = 'If there is: your CDN is likely caching the image without varying on the *Accept* header. ' .
                 'Make sure your CDN is configured to cache by the *Accept* request header (or to forward it to the origin).';

            if (PlatformInfo::isApache()) {
                $log[] = 'If not: it seems the .htaccess rules ';
                $log[] = 'just arent working on your system. Please report this at https://github.com/mboisvertdupras/magic-convert/issues';
            } elseif (PlatformInfo::isNginx()) {
                 $log[] = 'Also, as you are on Nginx, use the **native Nginx support**{: .ok}: head over to the *Nginx* ' .
                     'section on the Magic Convert settings page, install the generated config and run the live test there.';
            } else {
                $log[] = 'If not: it seems that there is something ' .
                    'in the *.htaccess* rules generated by Magic Convert that are not working. ' .
                    'Please report this at https://github.com/mboisvertdupras/magic-convert/issues';
            }
