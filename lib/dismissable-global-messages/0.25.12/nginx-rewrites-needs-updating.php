...
DismissableGlobalMessages::printDismissableMessage(
    'info',
    '<p>Magic Convert needed to rename its configuration files to contain random characters - for security reasons. ' .
        ...
        '<p>In short, by editing the Magic Convert rules set up in your server context (usually found in /etc/nginx/sites-available), if you pass "&hash=' .  Paths::getConfigHash() . '" to the converter script, the converter script can read the config file directly, without resorting to finding it by inspecting the files in the directory.</p>',
    $msgId,
    [
        ['text' => 'Ok'],
    ]
