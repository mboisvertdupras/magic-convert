...
Parallelized, multi-format (WebP + AVIF) image conversion for WordPress.

Magic Convert is a fork of [WebP Express](https://github.com/rosell-dk/webp-express) by Bjørn Rosell, extended with three headline goals: parallelized bulk conversion for large media libraries (50k+ images), AVIF output alongside WebP, and native, settings-aware nginx config generation.

Developed at [mboisvertdupras/magic-convert](https://github.com/mboisvertdupras/magic-convert). Original WebP Express project: [rosell-dk/webp-express](https://github.com/rosell-dk/webp-express). Huge thanks to Bjørn Rosell and the WebP Express contributors — the technical foundation, conversion engine, and most of the documentation below come from their work.

The technical documentation that follows (server setup, nginx instructions, FAQ, etc.) is inherited from WebP Express and is being progressively updated for Magic Convert.
...
5. (Optional) Bulk convert all images, either in the admin ui or using WP CLI (command: "magic-convert")

### Configuring
You configure the plugin in *Settings > Magic Convert*.
...
2. If you haven't saved yet, click "Save settings". This will put redirection rules into .htaccess files in the relevant directories (typically in uploads, themes and wp-content/magic-convert/webp-images, depending on the "Scope" setting)
...
2. By using WP CLI (command: "magic-convert").
...
3. Set up a cron job, which executes `wp magic-convert convert` regularily
...
`wp magic-convert --help`. Displays the available commands
`wp magic-convert convert --help`. Displays the available options for the "convert" command.

A few examples:
`wp magic-convert convert`: Creates webp images for all unconverted images
`wp magic-convert convert --reconvert`: Also convert images that are already converted
`wp magic-convert convert themes`: Only images in the themes folder
`wp magic-convert convert uploads/2021`: Only images in the "2021" folder inside the uploads folder
`wp magic-convert convert --only-png`: Only the PNG images
`wp magic-convert convert --quality=50`: Use quality 50 (instead of what was entered in settings screen)
`wp magic-convert convert --converter=cwebp`: Specifically use cwebp converter.
...
- **The native way (generated nginx rules):** install the generated include and let nginx negotiate the format. This is the recommended path when you can edit (or include) nginx config.
...
2. **Pick your setup and install the file(s).** The panel offers two ready-to-include artifacts, each with Copy and Download buttons and per-host install instructions:
   - **Standard (two files)** — `magic-convert-maps.conf` goes in the **http** context (e.g. `/etc/nginx/conf.d/`), and `magic-convert-server.conf` is `include`d in your site's **server** context (e.g. via `/etc/nginx/snippets/`). This is the cleanest, fastest shape.
   - **Single file (control panels)** — `magic-convert.conf` is one self-contained server-context file for GridPane / RunCloud / Plesk-style hosts whose per-site include slot cannot hold http-context `map`s. Paste it into the panel's nginx include slot.
...
The generated config embeds a `location = /magic-convert-rules-version` marker that returns a fingerprint of the rule-affecting settings. Whenever you change a setting that affects the nginx rules, Magic Convert:

- shows a dismissable **"your nginx rules need updating"** notice, and
- (on a daily, silent background check) compares the fingerprint your installed rules report over HTTP against your current settings, re-arming that notice if they have drifted apart.

When you see the notice, just re-open the nginx panel, copy/download the updated file(s), and reload nginx. The **Run live test** button's drift step confirms when you are back in sync.

> **Heads up about `add_header`:** the generated location blocks set `add_header Vary Accept;`. In nginx, `add_header` inside a `location` *replaces* any headers inherited from the surrounding server/http context. If you rely on inherited headers (HSTS, CSP, X-Frame-Options…), re-declare them inside these location blocks, or they will be dropped for image responses. The generated files carry a prominent comment warning about this.

#### Manual configuration (advanced)

You normally never need this — the panel is the source of truth and fills in everything (hash, paths, cache dirs, image types, AVIF, extension mode). But if you want to read or hand-tune the rules, **the generated artifacts in the nginx panel are the canonical reference.** Download `magic-convert-maps.conf` + `magic-convert-server.conf` (or the single `magic-convert.conf`), read the header comment block (it lists every setting reflected in the rules), and adapt from there.

Notes for manual setups:

- The rules look for converted files under the per-format cache trees (`…/magic-convert/webp-images/…` and `…/magic-convert/avif-images/…`) and as siblings in "mingled" mode. Keeping *General → File extension* on *Append ".webp"* is the most broadly compatible choice.
- If you moved `wp-content` to a non-standard location, regenerate from the panel — the generated `wp-content=` parameter and cache paths are adjusted automatically.
- Beware of permalink hacks based on `rewrite` (rather than `try_files`) for routing missing files to `index.php`; you may need to adjust as described [here](https://wordpress.org/support/topic/nginx-server-404-not-found-when-convert-test-images/page/2/#post-11952444).
