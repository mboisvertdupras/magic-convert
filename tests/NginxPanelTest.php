...
/**
 * Pure-part tests for the Phase 3.2 nginx admin experience:
 *   - the artifact-key WHITELIST + normalization (the only gate between request input and
 *     artifact dispatch in NginxDownload),
 *   - download filenames,
 *   - the fingerprint-CHANGE decision that drives the update notice,
 *   - the gating of NginxRulesNotice::arm() (nginx + changed).
 *
 * UI partials (nginx-panel.inc) are render-glue and deliberately untested here — all logic lives
 * in these pure methods. WordPress is stubbed at the namespace seam (see NginxPanelWpStubs.php)
 * so the armed-notice path can be exercised without bootstrapping WordPress.
 */
class NginxPanelTest extends TestCase
{
    protected function setUp(): void
    {
        // Reset the in-memory option store the WP stubs back, so each test starts clean.
        \MagicConvert\Tests\reset_wp_stub_options();
    }

    // --- artifact-key whitelist --------------------------------------------------------------
    ...
    {
        // 'a' (the two-file standard) defaults to the server file; 'b' is the single file.
    ...
    public function testDownloadFilenames(): void
    {
        ...
        // Unknown -> safe default (never an empty/odd name).
    ...
    {
        // No prior state => nothing installed to drift from => not a "change".
    ...
    {
        // A corrupt/empty previous fingerprint should surface the notice, not hide it.
    ...
    // --- notice gating (NginxRulesNotice::arm) ------------------------------------------------

    public function testArmDoesNothingWhenNotNginx(): void
    {
        $armed = NginxRulesNotice::arm($this->record('abc'), $this->record('def'), /* isNginx */ false);
    ...
    {
        $armed = NginxRulesNotice::arm($this->record('abc'), $this->record('abc'), /* isNginx */ true);
        $this->assertFalse($armed);
        $this->assertNotContains(NginxRulesNotice::MESSAGE_ID, $this->queuedGlobalMessageIds());
    }
    ...
    {
        $armed = NginxRulesNotice::arm(null, $this->record('abc'), /* isNginx */ true);
        $this->assertFalse($armed);
        $this->assertNotContains(NginxRulesNotice::MESSAGE_ID, $this->queuedGlobalMessageIds());
    }
    ...
    {
        $armed = NginxRulesNotice::arm($this->record('abc'), $this->record('def'), /* isNginx */ true);
    ...
    {
        // Arm -> dismiss -> arm again on a NEW change: the stable id is re-queued (reappears).
    ...
    /** Read the current global-message queue via the same State the production code uses. */
